...
struct locate_last_if_not_fn {
    public:
        ...
        static constexpr decltype(auto) operator()(Tuple&& tuple, Pred pred, Proj proj = {}) {
            auto&& result = find_last_if_not(std::forward<Tuple>(tuple), std::ref(pred), std::ref(proj));
...
inline constexpr _unspecified::locate_last_if_not_fn locate_last_if_not{};
